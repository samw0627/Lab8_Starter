describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (var i = 1; i< prodItems.length; i++){
      ...
      const innerTxt = await cardCount.getProperty('innerText');
      expect(await innerTxt.jsonValue()).toBe("20");
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    var cart2 =  JSON.parse(cart);
    for(var i = 0; i< cart2.length; i++){
      expect(cart2[i]).toBe(i+1);
    }
  });
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount2 = await page.$('#cart-count')
    const innerText3 = await cartCount2.getProperty('innerText');
    expect(await innerText3.jsonValue()).toBe("0");
  }, 10000);
